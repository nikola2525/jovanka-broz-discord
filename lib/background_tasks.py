...
async def new_atb_apps(bot):
    ...
    apps = DBM.coll_new_apps.find().sort([('_id', -1)])  # Reverse Sort
    ...
    if apps:
        for app in apps:
            ...
            embed.add_field(
                ...
                value='```{}```'.format(app['personal_other'][:1018] if app['personal_other'] else '<Nista nije uneto>'),
                inline=False
            )

            embed.add_field(
                name='Ime maina i spec',
                value='{} ({})'.format(app['main_name'] if app['main_name'] else '<Nista nije uneto>', app['class_and_spec'] if app['class_and_spec'] else '<Nista nije uneto>'),
                inline=True
            )
            ...
            embed.add_field(
                ...
                value='```{}```'.format(app['game_additional'][:1018] if app['game_additional'] else '<Nista nije uneto>'),
                inline=False
            )
...
async def guild_roster_check(bot):
    ...
    guilds_to_check = list(set([doc["guild_name"]+'-'+doc["realm_slug"] async for doc in DBM.db_main['guild_roster_subs'].find({})]))
    for guild in guilds_to_check:
        ...
        bnet_guild_roster = await u.get_guild_roster(realm_slug, guild_name)
        # Get the roster from the database
        db_guild_roster = DBM.db_main['guild_roster_current'].find({
            "guild_name": guild_name,
            "realm_slug": realm_slug
        })
        # Only filter out the member names
        bnet_guild_roster_short = [member['character']['name'] for member in bnet_guild_roster]
        guild_info = {
            "guild_name": guild_name,
            "realm_slug": realm_slug,
        ...
        try:
            ...
            print(f'Guild {guild_name.title()} on {realm_slug.title()} has not been found in internal DB, updating. . .')
            await DBM.guild_update(guild_info)
            continue
        # Check if any differences are present
        members_left = [member for member in db_guild_roster['guild_roster'] if member not in bnet_guild_roster_short]
        members_joined = [member for member in bnet_guild_roster_short if member not in db_guild_roster['guild_roster']]
        ...
        if members_left or members_joined:
            ...
            await guild_sub_announcer(bot, guild_info, members_joined, members_left)
        else:
            print(f'No roster changes detected in {guild_name.title()} on {realm_slug.title()}.')
...
async def guild_sub_announcer(bot, guild_info, joined, left):
    ...
    subbed_users = DBM.db_main['guild_roster_subs'].find({
